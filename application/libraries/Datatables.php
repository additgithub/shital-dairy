  ...
  */
  class Datatables
  {
    ...
    */
    ...
    {
      $this->where[] = array($key_condition, $val, $backtick_protect);
      $this->ci->db->like("LOWER($key_condition)", strtolower($val), $backtick_protect);
      return $this;
    }
    ...
    */
    protected function get_paging()
    {
      ...
      $this->ci->db->limit(($iLength != '' && $iLength != '-1')? $iLength : '', ($iStart)? $iStart : 0);
    }
    ...
    */
    ...
    {
      $scol_arr = array();
      if($this->ci->input->post('sColumns')):
          $scol_arr = explode(',', $this->ci->input->post('sColumns')); //!array_filter($array)
      endif;
        
      if($this->check_mDataprop())
        $mColArray = $this->get_mDataprop();
      elseif(array_filter($scol_arr))
      ...
      if($sSearch != '')
        for($i = 0; $i < count($mColArray); $i++)
          if($this->ci->input->post('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns))
            $sWhere .= "LOWER(".$this->select[$mColArray[$i]].")" . " LIKE '%" . strtolower($sSearch) . "%' OR ";
        ...
        if(isset($_POST['sSearch_' . $i]) && $this->ci->input->post('sSearch_' . $i) != '' && in_array($mColArray[$i], $columns))
        {
          ...
          foreach($miSearch as $val)
          {
            if(preg_match("/(<=|>=|=|<|>)(\s*)(.+)/i", trim($val), $matches))
              ...
              $this->ci->db->where("LOWER(".$this->select[$mColArray[$i]].")".' LIKE', '%'.strtolower($val).'%');
          }
        }
      }
